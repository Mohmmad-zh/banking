...
function DebitDeposit(props) {
  const { show, setShow, cardId } = props
  const { DebitDeposit } = useContext(BankContext)
  return (
    <Modal show={show} onHide={() => setShow(false)} >
      <Form onSubmit={(e) => DebitDeposit(e, cardId)} style={{backgroundColor:"#f0efeb"}}>
        <Modal.Header closeButton>
          <Modal.Title> Debit Deposit</Modal.Title>
        </Modal.Header>
        <Modal.Body>
